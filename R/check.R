CheckInputRegression=function(xdata, ydata, Lambda=NULL, pi_list=seq(0.6,0.9,by=0.01),
                              ...
                              verbose=TRUE){
  # List of arguments
  myargs=c("xdata", "ydata", "Lambda", "pi_list", "K", "tau", "seed",
  ...
  if (any(pi_list>0.5)&any(pi_list<1)){
    ...
    stop("Invalid input for argument 'pi_list'. The values in the vector must be greater than 0.5 and lower than 1. To consider thresholds below 0.5, unstable features can be kept in the model, i.e. with selection proportions greater than or equal to (1-pi).")
  }
...
CheckInputNetwork=function(data, pk=NULL, Lambda=NULL, lambda_other_blocks=NULL,
                           pi_list=seq(0.6,0.9,by=0.01), K=100, tau=0.5, seed=1,
  ...
  if ((length(pk)==1)&(!is.null(lambda_other_blocks))){
